...
function serverEnv(dataDir, extra = {}) {
  return {
    ...
    TRUST_PROXY: '0',
    // SITE_URL نباید از backend/.env توسعه‌دهنده به سرورِ سندباکس نشت کند.
    // این متغیر آدرسِ بازگشتِ درگاه پرداخت را می‌سازد (routes/orders.js:96). اگر
    // مقدارش مبدأ دیگری باشد — مثلاً فرانت‌اندِ Next روی ۳۰۰۱ — تستی که
    // paymentUrl را باز می‌کند، پرداخت را روی *آن* سرور و دیتابیسِ **واقعی**
    // انجام می‌دهد، نه روی سندباکسِ خودش. آن سفارش آنجا وجود ندارد، پس سفارشِ
    // سندباکس تا آخر pending_payment می‌ماند و ده‌ها تستِ بعدی آبشاری می‌شکنند.
    SITE_URL: extra.PORT ? `http://127.0.0.1:${extra.PORT}` : '',
    ...extra
  };
}
