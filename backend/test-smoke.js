...
const child = spawn(process.execPath, [path.join(__dirname, 'server.js')], {
  ...
  env: {
    ...
    // آدرسِ بازگشتِ درگاه باید روی *همین* سرورِ تست بیفتد.
    // خط ۳۹۴ و بعدش paymentUrl را واقعاً باز می‌کنند تا پرداختِ آزمایشی انجام
    // شود. از وقتی callbackUrl از SITE_URL ساخته می‌شود (routes/orders.js:96)،
    // هر مقداری که در backend/.env نشسته بود به این پروسه ارث می‌رسید و پرداخت
    // روی دیتابیسِ واقعی انجام می‌شد نه سندباکس — سفارشِ تست pending می‌ماند و
    // دوازده تستِ وابسته پشتِ سرِ هم می‌شکستند.
    SITE_URL: BASE
  },
